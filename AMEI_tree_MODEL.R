...
model_uncertainty <- function(df){

  layer      <- as.integer(df[['layer_lvl']])
  model_name <- df[['model_name']]
  
  print(paste0("CART tree is being generated for the model ...", model_name, " layer ", layer))
  
  
  #filter by model
  df_model_uncer <- raw_data[raw_data$SLLB==layer & raw_data$model_name==model_name,]
  
  #get mean
  TSLD_ensemble_mean <- mean(df_model_uncer$TSLD, na.rm=T)
  #get median 
  ...
  tree <- rpart(TSLD_error~ soil + lai + aw, data = df_model_uncer, 
  ...
  pdf(paste0(path_output,"/model_CART_uncertainty/CART_tree_model-",model_name,"_layer-", layer,".pdf"))
  
  tree_plot <- rpart.plot(tree.pruned, 
                          #type = 4, 
                          extra = 101,
                          digits=-3,
                          main=paste0("Driver: ", model_name, " Layer: ", layer))
  dev.off()
  #ggsave(paste0(path_output,"/model_CART_uncertainty/CART_tree_",model_name,"_", layer,".jpg"),
  #       dpi =300,tree_plot,width = 17, height = 13)
}


apply(runs_df,MARGIN=1,FUN=model_uncertainty)
